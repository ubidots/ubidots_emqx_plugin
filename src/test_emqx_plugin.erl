...
on_client_subscribe(ClientInfo = #{clientid := ClientId, username := UserName}, Properties, TopicFilters, _Env) ->
    io:format("Client(~s), ClientInfo:~n~p~n, will subscribe: ~p~n~p~n", [ClientId, ClientInfo, Properties, TopicFilters]),
    NewTopicFilters = lists:map(fun(TopicFilter) -> emqx_topic_changer:add_users_topic(UserName, TopicFilter) end, TopicFilters),
    io:format("NEW TOPICS ~n~p~n", [NewTopicFilters]),
    {ok, NewTopicFilters}.
...
on_message_delivered(_ClientInfo = #{clientid := ClientId}, #message{topic = Topic} = Message, _Env) ->
